...
class Pixel:
    ...
    def __init__(self,canv,i,j,nrow,ncol,scale,icolor,vector=[0,0]): 
        ...
        x=self.j*self.scale
        y=self.i*self.scale
        x2=x+self.scale     #calculates position of pixel and creates rectangle
        y2=y+self.scale
        self.lol=canv.create_rectangle(x,y,x2,y2,fill = Pixel.color[self.icolor])
    # Remove the pass statement and implement the __init__ method as described in the PDF.


    def __str__(self):
        return f"{self.nrow}{self.icolor} {self.i}{self.j}" #returns string of pixel position
    # Remove the pass statement and implement the __str__ method as described in the PDF.


    def delete(self):
        self.canv.delete(self.lol)  #clears the canvas
    # Remove the pass statement and implement the delete method as described in the PDF.
   

    def left(self):
        self.vector = [0,-1] #sets pixel movement vector left
    # Remove the pass statement and implement the left method as described in the PDF.
        

    def right(self):
        self.vector = [0,1] #sets pixel movement vector right
    # Remove the pass statement and implement the right method as described in the PDF.


    def up(self):
        self.vector = [-1,0] #sets pixel movement vector up
    # Remove the pass statement and implement the up method as described in the PDF.


    def down(self):
        self.vector = [1,0] #sets pixel movement vector down
    # Remove the pass statement and implement the down method as described in the PDF.


    def next(self):
        """
        changes pixels position based off changes in the vector created by left right up down, and updates the canvas.
        """
        self.i += self.vector[0] 
        self.j+= self.vector[1]  #moves pixel to its next position indicated by the vector
        x=self.j*self.scale
        y=self.i*self.scale #changes what row/col the pixel is in based off vector
        x2=x+self.scale
        y2=y+self.scale
        self.canv.coords(self.lol,x,y,x2,y2) #updates location on canvas
